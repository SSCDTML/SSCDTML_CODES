...
def testGenerator(image_path, target_size = (512,512,3), flag_multi_class = True, as_gray = True):
...
def saveResult(file_name, save_path, npyfile, vid, flag_multi_class = True, num_class = 4):

    for i, item in enumerate(npyfile):
        img = labelVisualize(num_class,COLOR_DICT, np.array(item)) if flag_multi_class else item[:,:,0]
        img = img.astype(np.uint8)
        file_name = "{}_predict_{}.png".format(file_name, vid)
